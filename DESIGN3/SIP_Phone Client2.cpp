...
void handle_submit() {
  ...
  if (!new_subnet.fromString(subnet_str)) {
    server.send(400, "text/plain", "Invalid subnet mask");
    return;
  }
  if (!new_gateway.fromString(gateway_str)) {
    server.send(400, "text/plain", "Invalid gateway address");
    return;
  }
  if (!new_dns1.fromString(dns1_str)) {
    server.send(400, "text/plain", "Invalid primary DNS address");
    return;
  }
  if (!new_dns2.fromString(dns2_str)) {
    server.send(400, "text/plain", "Invalid secondary DNS address");
    return;
  }
